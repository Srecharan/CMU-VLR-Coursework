...
def train(args, model, optimizer, scheduler=None, model_name='model'):
    ...
    for epoch in range(args.epochs):
        for batch_idx, (data, target, wgt) in enumerate(train_loader):
            ...
            shift_logits = output - torch.max(output, 1, keepdim=True)[0]
            exp_shifted = torch.exp(shift_logits)
            predictions = exp_shifted / (torch.sum(exp_shifted, 1, keepdim=True))

            # Add small constant for numerical stability 
            numerical_stability = 1e-7

            # Calculate weighted binary cross entropy
            log_predictions = torch.log(predictions + numerical_stability)
            log_complement = torch.log(1 - predictions + numerical_stability)
            binary_ce = -(target * log_predictions + (1 - target) * log_complement)

            # Apply difficulty weights and reduce
            loss = torch.mean(binary_ce * torch.unsqueeze(wgt, 1))
